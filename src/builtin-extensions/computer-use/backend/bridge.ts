...
import type { AgentToolResult, AgentToolUpdateCallback, ExtensionContext } from "./pi-shim.js";
import { canRetryInForeground, outcomeAfterCheck, outcomeAfterObservedValues, prepareAction, type ActionState, type PreparedAction } from "./actions.js";
import { getComputerUseConfig, isHeadlessMode, loadComputerUseConfig } from "./config.js";
import { noteAfterAct, noteFromLook, noteRegionKeyForRef, renderNote, type WindowNote } from "./note.js";
...
import { applyOutputEnvelope, boundToolError, clearStoredOutputs, readStoredOutput, UI_TEXT_PAGE_CHARS } from "./output.js";
import { AGENT_TOOL_NAMES, CLI_COMMAND_NAME, type ActParams, type ExpandUiParams, type ImageMode, type InspectUiParams, type FindParams, type ObserveParams, type ObserveTargetParams, type ReadTextParams, type RootSelector, type SearchUiParams, type UiAction, type WaitForParams } from "./contract.js";
import { toFiniteNumber } from "./platform/coerce.js";
import { currentPlatformBackend } from "./platform/index.js";
import type { FramePoints, HelperActPerformed, HelperActResult, NativeInputDelivery, PlatformActRequest, PlatformApp as HelperApp, PlatformDiagnostics, PlatformFrontmostResult as FrontmostResult, PlatformRoot as HelperWindow } from "./platform/types.js";
import type { PermissionStatus } from "./permissions.js";
import { ResourceScheduler } from "./runtime.js";
import { scoreWindow, shouldPreferForegroundModalWindow } from "./root-selection.js";
import { SavedStates, type CurrentCapture, type CurrentTarget, type OperationState } from "./state.js";
import { changesBetween, renderChanges, stabilizeRefs } from "./view.js";
export type { ActParams, ExpandUiParams, ImageMode, InspectUiParams, FindParams, MouseButtonName, ObserveParams, ObserveTargetParams, ReadTextParams, RootSelector, SearchUiParams, StateTargetParams, UiAction, WaitForParams } from "./contract.js";
...
export function reconstructStateFromBranch(ctx: ExtensionContext): void {
	...
	for (const entry of [...ctx.sessionManager.getBranch()].reverse()) {
		...
		let rawDetails = message.details as any;
		if (message.toolName === "cli") {
			// Result routed through the _computer_use cli command: the backend
			// details are nested under details.builtin.details.
			const builtin = rawDetails?.builtin;
			if (builtin?.name !== CLI_COMMAND_NAME) continue;
			rawDetails = builtin.details;
		} else if (!AGENT_TOOL_NAMES.has(message.toolName)) {
			continue;
		}
