...
Extensions can register custom model providers via `pizza.registerProvider()`. This enables:
...
export default function (pizza: ExtensionAPI) {
  ...
  pizza.registerProvider("my-provider", {
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Pizza waits for the factory before startup continues, so the provider is available during interactive startup and to `pizza --list-models`.
...
Use `pizza.unregisterProvider(name)` to remove a provider that was previously registered via `pizza.registerProvider(name, ...)`:
...
// Later, remove it
pizza.unregisterProvider("my-llm");
```
...
```typescript
pizza.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "MY_API_KEY",
...
Credentials are persisted in `~/.pizza/agent/auth.json`:
