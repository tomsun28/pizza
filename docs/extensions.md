...
> **Placement for /reload:** Put extensions in `~/.pizza/agent/extensions/` (global) or `.pizza/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
