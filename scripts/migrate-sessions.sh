#!/usr/bin/env bash
#
# Migrate sessions from ~/.glm/agent/*.jsonl to proper session directories.
# This fixes sessions created by the bug in v0.30.0 where sessions were
# saved to ~/.glm/agent/ instead of ~/.glm/agent/sessions/<encoded-cwd>/.
#
...
AGENT_DIR="${GLM_AGENT_DIR:-$HOME/.glm/agent}"
DRY_RUN=false
