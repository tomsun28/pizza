...
async function checkDirtyRepo(
	...
	const { stdout, code } = await pizza.exec("git", ["status", "--porcelain"]);
...
export default function (pizza: ExtensionAPI) {
	pizza.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(pizza, ctx, action);
	});

	pizza.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(pizza, ctx, "fork");
	});
}
