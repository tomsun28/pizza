 ...
 *
 * Usage:
 *   pizza -e ./ssh.ts --ssh user@host
 *   pizza -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (pizza: ExtensionAPI) {
	pizza.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	pizza.on("session_start", async (_event, ctx) => {
		...
		const arg = pizza.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
