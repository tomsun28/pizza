...
export default function (pizza: ExtensionAPI) {
	...
	pizza.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "Pizza RPC Demo (new session)" : "Pizza RPC Demo");
	...
	pizza.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	pizza.on("turn_end", async (_event, ctx) => {
	...
	pizza.registerCommand("rpc-input", {
	...
	pizza.registerCommand("rpc-editor", {
	...
	pizza.registerCommand("rpc-prefill", {
