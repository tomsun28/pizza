 ...
 *
 * Usage: pizza --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "pizza";
...
export default function (pizza: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like pizza-doom would need)
	pizza.registerCommand("overlay-animation", {
	...
	pizza.registerCommand("overlay-anchors", {
	...
	pizza.registerCommand("overlay-margins", {
	...
	pizza.registerCommand("overlay-stack", {
	...
	pizza.registerCommand("overlay-overflow", {
	...
	pizza.registerCommand("overlay-edge", {
	...
	pizza.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	pizza.registerCommand("overlay-maxheight", {
	...
	pizza.registerCommand("overlay-sidepanel", {
	...
	pizza.registerCommand("overlay-toggle", {
	...
	pizza.registerCommand("overlay-passive", {
	...
	pizza.registerCommand("overlay-focus", {
	...
	pizza.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/Pizza/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/Pizza\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like pizza-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (pizza-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
