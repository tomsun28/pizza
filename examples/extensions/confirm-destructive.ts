...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "pizza";

export default function (pizza: ExtensionAPI) {
	pizza.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
