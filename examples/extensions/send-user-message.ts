 ...
 *
 * Demonstrates pizza.sendUserMessage() for sending user messages from extensions.
 * Unlike pizza.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (pizza: ExtensionAPI) {
	...
	pizza.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			pizza.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	pizza.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				pizza.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	pizza.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				pizza.sendUserMessage(args, { deliverAs: "followUp" });
