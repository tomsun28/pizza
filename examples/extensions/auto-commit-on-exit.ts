...
export default function (pizza: ExtensionAPI) {
	pizza.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await pizza.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[pizza] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await pizza.exec("git", ["add", "-A"]);
		const { code: commitCode } = await pizza.exec("git", ["commit", "-m", commitMessage]);
