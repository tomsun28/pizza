...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pizza-ext-test-"));
	...
	const extensionCode = `
		export default function(pizza) {
			pizza.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json pizza field", async () => {
	...
	it("package.json with pizza field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without pizza field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ pizza: { extensions: ["./entry.ts"] } }));
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(pizza) {
				pizza.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(pizza) {
				pizza.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(pizza) {
				pizza.on("agent_start", async () => {});
				pizza.on("tool_call", async (event) => undefined);
