...
Content`,
		...
		it("should auto-discover root markdown skills from .pizza skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .pizza", async () => {
			const extDir = join(tempDir, ".pizza", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".pizza", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json pizza.extensions in extensions setting", async () => {
			// Create a package with pizza.extensions in package.json
			...
			// Should find the extensions declared in package.json pizza.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".pizza", "agent");
		...
		it("should dedupe user skill entries when ~/.pizza/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pizza auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pizza\n");

			const skillDir = join(tempDir, ".pizza", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with pizza manifest", async () => {
	...
	describe("source parsing", () => {
		(process.env.PIZZA_OFFLINE === "1" ? it.skip : it)("should emit progress events on install attempt", async () => {
			const events: ProgressEvent[] = [];
			packageManager.setProgressCallback((event) => events.push(event));
		...
		(process.env.PIZZA_OFFLINE === "1" ? it.skip : it)("should recognize github URLs without git: prefix", async () => {
			const events: ProgressEvent[] = [];
			packageManager.setProgressCallback((event) => events.push(event));
	...
	describe("pattern filtering in pizza manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json pizza.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should not check package updates when offline", async () => {
			// Update checks disabled - always returns empty array
